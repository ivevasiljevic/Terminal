<div class="contentCat">
	<nav class="breadcrumb">
  		...
  		<span class="breadcrumb-item active">Komponente</span>
	</nav>
	<div class="category">
		<?php
		...
		$query = $conn->query("SELECT file_path,product_name,price FROM product inner join image on product.product_id = image.product_id inner join category on product.category_id = category.category_id
			WHERE category.category_name = 'graficka' OR category.category_name = 'maticna' OR category.category_name = 'procesor' OR category.category_name = 'memorija' OR category.category_name = 'napajanje' OR category.category_name = 'ssd'");
