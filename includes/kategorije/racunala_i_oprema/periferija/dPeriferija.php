<div class="contentCat">
	<nav class="breadcrumb">
  		...
  		<span class="breadcrumb-item active">Periferija</span>
	</nav>
	<div class="category">
		<?php
		...
		$query = $conn->query("SELECT file_path,product_name,price FROM product inner join image on product.product_id = image.product_id inner join category on product.category_id = category.category_id
			WHERE category.category_name = 'zvucnik' OR category.category_name = 'mis' OR category.category_name = 'tipkovnica' OR category.category_name = 'monitor' OR category.category_name = 'hdd_x'");
